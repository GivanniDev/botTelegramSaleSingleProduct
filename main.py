...
admin_keyboard.row("🔥 Прогнозы", "🎓 Поддержка")
admin_keyboard.row("🎮 Подписаться на канал")
admin_keyboard.row("👾 Использовать прогноз")
...
user_keyboard.row("🔥 Прогнозы", "🎓 Поддержка")
user_keyboard.row("🎮 Подписаться на канал")
user_keyboard.row("👾 Использовать прогноз")
ignor_command = ["🎁 Купить", "ℹ️ FAQ", "ℹ️ Изменить FAQ 🖍", "📘 Добавить товар", "📙 Удалить товар",
                 "📗 Добавить товары", "📕 Удалить все товары", "🔏 Изменить QIWI кошелёк", "🔐 Проверить QIWI кошелёк"]
####################################################################################################
...
with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
    ...
    try:
        ...
        cur.execute(
            "CREATE TABLE items(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, description TEXT, price INT, data TEXT)")
    ...
    try:
        ...
        cur.execute(
            "CREATE TABLE buyers(users TEXT, iditem TEXT, comment TEXT, amount TEXT, receipt TEXT, randomnum, data TEXT)")
...
def send_text(message):
    ...
    if message.text == "🔥 Прогнозы":
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            x = 0
            if len(idss) >= 1:
                with sqlite3.connect("shopBD.sqlite",
                    ...
                    while True:
                        ...
                        if row == None:
                            break
                        if x <= 10:
                            keyboard.add(types.InlineKeyboardButton(text=(str(row[1]) + " - " + str(row[3]) + " руб"),
                                                                    callback_data="b_select_item_" + str(
                                                                        row[0]) + "|0"))
                if len(idss) > 10:
                ...
                try:
                    bot.send_message(message.chat.id, "🎁 Извиняемся, в данное время товары отсутствуют.",
    ...
    if message.text == "🎮 Подписаться на канал":
        keyboard = types.InlineKeyboardMarkup()
        url_button = types.InlineKeyboardButton(text="🏆 Подписаться", url="https://t.me/winXchanel")
        keyboard.add(url_button)
        bot.send_message(message.from_user.id,"🥇 Переходи в наш Telegram-канал, чтобы быть вкурсе всех спортивных событий", reply_markup=keyboard)
    if message.text == "👾 Использовать прогноз":
        keyboard = types.InlineKeyboardMarkup()
        url_button = types.InlineKeyboardButton(text="🎲 Сделать ставку", url="https://aff1xstavka.com/L?tag=s_847171m_1341c_519581&site=847171&ad=1341&r=line/Esports/")
        keyboard.add(url_button)
        bot.send_message(message.from_user.id, "Не ведись на лохотрон🙅Ставь у проверенного букмекера👇", reply_markup=keyboard)
    if message.text == "🎓 Поддержка":
        ...
        selectQiwi = request.get("https://edge.qiwi.com/payment-history/v2/persons/" + checkQiwi[0] + "/payments",
                                 params=parameters)
            ...
            if len(idss) >= 1:
                with sqlite3.connect("shopBD.sqlite",
                    ...
                    while True:
                        ...
                        if x <= 10:
                            keyboard.add(types.InlineKeyboardButton(text=(str(row[1]) + " - " + str(row[3]) + " руб"),
                                                                    callback_data="r_select_item_" + str(
                                                                        row[0]) + "|0"))
                if len(idss) > 10:
    ...
    if call.data == "r_nextPage|" + str(remover):
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            try:
                ...
                for a in range(remover, len(idss)):
                    if x < 10:
                        keyboard.add(types.InlineKeyboardButton(text=(str(name[a]) + " - " + str(amounts[a]) + " руб"),
                                                                callback_data="r_select_item_" + str(
                                                                    idss[a]) + "|" + str(remover)))
                    x += 1
                if remover + 9 >= len(idss):
                    keyboard.add(types.InlineKeyboardButton(text="⬅️Назад⬅️",
                                                            callback_data="r_previousPage|" + str(remover - 10)))
                else:
                    next_keyboard = types.InlineKeyboardButton(text="➡️Далее➡️",
                                                               callback_data="r_nextPage|" + str(remover + 10))
                    number_keyboard = types.InlineKeyboardButton(text=str(remover)[:1], callback_data=".....")
                    previous_keyboard = types.InlineKeyboardButton(text="⬅️Назад⬅️",
                                                                   callback_data="r_previousPage|" + str(remover - 10))
        ...
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            try:
                ...
                if remover <= 0:
                    keyboard.add(
                        types.InlineKeyboardButton(text="➡️Далее➡️", callback_data="r_nextPage|" + str(remover + 10)))
                else:
                    next_keyboard = types.InlineKeyboardButton(text="➡️Далее➡️",
                                                               callback_data="r_nextPage|" + str(remover + 10))
                    number_keyboard = types.InlineKeyboardButton(text=str(remover)[:1], callback_data=".....")
        ...
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            while True:
                ...
                if row[0] == msg:
                    keyboard = types.InlineKeyboardMarkup()
                    keyboard.add(
                        types.InlineKeyboardButton(text="📙 Удалить товар", callback_data="r_del_item_" + str(row[0])))
                    keyboard.add(types.InlineKeyboardButton(text="⬅️ Назад", callback_data="r_list_back_" + remover))
                    try:
                        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                              text="🏷 *Название товара:* `{0}`\n💵 *Стоимость товара:* `{1}`\n📜 *Описание товара:* `{2}`\n💾 *Данные товара:* `{3}`".format(
        ...
        try:
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text="📕 Вы отменили удаление товаров.")
        except requests.exceptions.ConnectionError:
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text="📕 Вы отменили удаление товаров.")
        ...
        with sqlite3.connect("shopBD.sqlite") as con:
            ...
            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="📕 Было удалено `{0}` товаров.".format(x), parse_mode="MARKDOWN")
            except requests.exceptions.ConnectionError:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text="📕 Было удалено `{0}` товаров.".format(x), parse_mode="MARKDOWN")
        if con:
    ...
    # Покупка товаров
        ...
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            try:
                ...
                if remover + 9 >= len(idss):
                    keyboard.add(types.InlineKeyboardButton(text="⬅️Назад⬅️",
                                                            callback_data="b_previousPage|" + str(remover - 10)))
                else:
                    next_keyboard = types.InlineKeyboardButton(text="➡️Далее➡️",
                                                               callback_data="b_nextPage|" + str(remover + 10))
                    number_keyboard = types.InlineKeyboardButton(text=str(remover)[:1], callback_data=".....")
                    previous_keyboard = types.InlineKeyboardButton(text="⬅️Назад⬅️",
                                                                   callback_data="b_previousPage|" + str(remover - 10))
                    keyboard.add(previous_keyboard, number_keyboard, next_keyboard)
        ...
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            try:
                ...
                if remover <= 0:
                    keyboard.add(
                        types.InlineKeyboardButton(text="➡️Далее➡️", callback_data="b_nextPage|" + str(remover + 10)))
                else:
                    next_keyboard = types.InlineKeyboardButton(text="➡️Далее➡️",
                                                               callback_data="b_nextPage|" + str(remover + 10))
                    number_keyboard = types.InlineKeyboardButton(text=str(remover)[:1], callback_data=".....")
        ...
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            while True:
                ...
                if row[0] == msg:
                    keyboard = types.InlineKeyboardMarkup()
                    keyboard.add(
                        types.InlineKeyboardButton(text="💰 Купить товар", callback_data="buy_item_" + str(row[0])))
                    keyboard.add(types.InlineKeyboardButton(text="⬅️ Назад", callback_data="b_list_back_" + remover))
                    try:
                        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                              text="🏷 *Название товара:* `{0}`\n💵 *Стоимость товара:* `{1}`\n📜 *Описание товара:* `{2}`".format(
                                                  row[1], row[3], row[2]),
                                              reply_markup=keyboard, parse_mode="MARKDOWN")
                    except requests.exceptions.ConnectionError:
                        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
        ...
        if con:
            con.close()
        if IdItems != 0:
            sendComment = "{0}|{1}.{2}.{3}.{4}".format(call.from_user.id, IdItems, randomNumber[0], randomChar[0],
                                                       randomFake[0])
            sendRequests = "https://qiwi.com/payment/form/99?extra%5B%27account%27%5D={0}&amountInteger={1}&amountFraction=0&extra%5B%27comment%27%5D={2}&currency=643&blocked%5B0%5D=sum&blocked%5B1%5D=comment&blocked%5B2%5D=account".format(
                sendNumber, sendItemsAmmo, sendComment)
            sendMessage = "📦 *Покупка товара*\n🥝 Для оплаты, нажмите кнопку ниже.\n*🥝 Менять поля НЕ НУЖНО*, всё уже указано.\n`(Ссылку нужно открывать через браузер, а не приложение qiwi)`\n➖➖➖➖➖➖➖➖➖➖➖➖➖\n🔅 *Номер:* `{0}`\n🔅 *Коментарий:* `{1}`\n🔅 *Сумма:* `{2}`\n➖➖➖➖➖➖➖➖➖➖➖➖➖\n🔄 После оплаты нажмите на кнопку `Проверить оплату`".format(
                sendNumber, sendComment, sendItemsAmmo)
            ...
            check_keyboard.add(types.InlineKeyboardButton(text="🔄 Проверить оплату",
                                                          callback_data=f"checkPay|{randomNumber[0]}|{sendItemsAmmo}"))
            try:
                if len(str(sendNumber)) < 10:
                            ...
                            for a in range(len(admin_id)):
                                try:
                                    bot.send_message(admin_id[a],
                                                     "❗️ Оплата не работает, срочно укажите новые киви данные ❗️")
                                except requests.exceptions.ConnectionError:
                                    bot.send_message(admin_id[a],
                                                     "❗️ Оплата не работает, срочно укажите новые киви данные ❗️")
                        else:
                    ...
                    try:
                        bot.send_message(call.message.chat.id, sendMessage, parse_mode="MARKDOWN",
            ...
            try:
                bot.send_message(call.message.chat.id, "❗️ Данный товар не был найден.", parse_mode="MARKDOWN",
                                 reply_markup=admin_keyboard)
            except requests.exceptions.ConnectionError:
                bot.send_message(call.message.chat.id, "❗️ Данный товар не был найден.", parse_mode="MARKDOWN",
                                 reply_markup=admin_keyboard)
        ...
        if allCheck:
            ...
            if yesOrNo:
                ...
                with sqlite3.connect("shopBD.sqlite",
                    ...
                    cur = con.cursor()
                    cur.executemany(
                        "INSERT INTO buyers(users, iditem, comment, amount, receipt, randomnum, data) VALUES (?, ?, ?, ?, ?, ?, ?)",
                        [(splitComment[0], splitComment[1], getCommentQiwi[howChar], getAmountQiwi[howChar],
                          getReceintQiwi[howChar], getDataPayer[1], getDateQiwi[howChar])])
                if con:
                    con.close()
                with sqlite3.connect("shopBD.sqlite") as con:
                    cur = con.cursor()
                ...
                try:
                    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                          text="✅ Благодарим вас за покупку. Ждём вас снова.\n`{0}`".format(sendBuy))
                    if len(admin_id) > 1:
                        for a in range(len(admin_id)):
                            bot.send_message(admin_id[a],
                                             f"💰 Пользователь @{call.from_user.username} купил товар на сумму {getAmountQiwi[howChar]} руб")
                    else:
                        bot.send_message(admin_id[0],
                                         f"💰 Пользователь @{call.from_user.username} купил товар на сумму {getAmountQiwi[howChar]} руб")
                except requests.exceptions.ConnectionError:
...
def add_item_data(message):
    ...
    if itemData not in ignor_command:
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            cur = con.cursor()
            cur.executemany("INSERT INTO items (name, description, price, data) VALUES (?, ?, ?, ?)",
                            [(itemName, itemDiscription, itemPrice, itemData)])
        if con:
            con.close()
        try:
    ...
    if itemPrices not in ignor_command:
        try:
            bot.send_message(message.from_user.id,
                             "📗 Введите данные товаров в столбик. Пример:\n`login:password`\n`login:password`\n`login:password`",
                             parse_mode="MARKDOWN")
        except requests.exceptions.ConnectionError:
            bot.send_message(message.from_user.id,
                             "📗 Введите данные товаров в столбик. Пример:\n`login:password`\n`login:password`\n`login:password`",
    ...
    if itemDatas not in ignor_command:
        with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
            ...
            for a in range(len(itemDatas)):
                cur.executemany("INSERT INTO items (name, description, price, data) VALUES (?, ?, ?, ?)",
                                [(itemNames, itemDiscriptions, itemPrices, itemDatas[a])])
                counter += 1
        if con:
            con.close()
        try:
...
def change_faq(message):
    ...
    if con:
        con.close()
    try:
        bot.send_message(message.from_user.id, "🔘 FAQ был обновлено")
    except requests.exceptions.ConnectionError:
...
def change_qiwi_token(message):
    ...
    try:
        ...
        parameters = {"rows": 5, "operation": "IN"}
        selectQiwi = request.get("https://edge.qiwi.com/payment-history/v2/persons/" + qiwi_login + "/payments",
                                 params=parameters)
        if selectQiwi.status_code == 200:
            with sqlite3.connect("shopBD.sqlite", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as con:
                cur = con.cursor()
                ...
                while True:
                    ...
                    if row == None:
                        break
                    cur.execute("UPDATE qiwi SET login = ?, token = ? WHERE login = ?",
                                (qiwi_login, message.text, row[0]))
            if con:
                con.close()
            try:
            ...
            try:
                bot.delete_message(chat_id=message.chat.id, message_id=message.message_id + 1)
                bot.send_message(message.from_user.id,
                                 "❌ QIWI токен не прошёл проверку. Код ошибки: " + str(selectQiwi.status_code))
            except requests.exceptions.ConnectionError:
                bot.delete_message(chat_id=message.chat.id, message_id=message.message_id + 1)
                bot.send_message(message.from_user.id,
                                 "❌ QIWI токен не прошёл проверку. Код ошибки: " + str(selectQiwi.status_code))
